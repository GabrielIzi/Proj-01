...
def insert_product(product_name, product_price, product_description, product_image):
    try:
        ...
        conn = pyodbc.connect(server=SQL_SERVER, database=SQL_DATABASE, user=SQL_USER, password=SQL_PASSWORD, driver='{ODBC Driver 18 for SQL Server}', trustservercertificate='yes')
        cursor = conn.cursor()
